...
from app.models import Member, Event, Pool, PoolRegistration, Booking, Role
...
class TestPoolManagement:
    ...
    def test_list_pools_with_event_manager(self, client, event_manager_user, temp_event_with_pool):
    ...
    def test_create_event_pool_get(self, client, event_manager_user, temp_event):
        """Test GET request to create event pool form."""
        with client.session_transaction() as sess:
            sess['_user_id'] = str(event_manager_user.id)
        
        response = client.get(f'/pools/create/event/{temp_event.id}')
        assert response.status_code == 200
    
    def test_create_event_pool_post_requires_csrf(self, client, event_manager_user, temp_event):
        """Test creating event pool requires CSRF token."""
        with client.session_transaction() as sess:
            sess['_user_id'] = str(event_manager_user.id)
        
        response = client.post(f'/pools/create/event/{temp_event.id}', data={
            'is_open': True
        })
...
class TestPoolRegistration:
    ...
    def test_register_member_success(self, client, temp_user, temp_event_with_pool):
        """Test successful member registration to pool."""
        with client.session_transaction() as sess:
            sess['_user_id'] = str(temp_user.id)
        
        pool = temp_event_with_pool.pool
    ...
    def test_register_member_already_registered(self, client, temp_user, temp_event_with_pool):
    ...
    def test_register_member_pool_closed(self, client, temp_user, temp_event):
        """Test registering for closed pool."""
        # Create closed pool
        closed_pool = Pool(event_id=temp_event.id, is_open=False)
    ...
    def test_unregister_member_success(self, client, temp_user, temp_event_with_pool):
...
class TestPoolActions:
    ...
    def test_delete_empty_pool_success(self, client, event_manager_user, temp_event):
        """Test successful deletion of empty pool."""
        # Create empty pool
        empty_pool = Pool(event_id=temp_event.id, is_open=True)
...
class TestPoolAdminRoutes:
    ...
    def test_admin_create_event_pool_requires_role(self, client, temp_user, temp_event):
        """Test admin pool creation requires Event Manager role."""
        with client.session_transaction() as sess:
            sess['_user_id'] = str(temp_user.id)
        
        response = client.post(f'/pools/admin/create_event_pool/{temp_event.id}')
        assert response.status_code in [403, 302]
    
    def test_admin_create_event_pool_success(self, client, event_manager_user, temp_event):
    ...
    def test_admin_add_member_to_pool_requires_role(self, client, temp_user, temp_event):
        """Test admin adding member to pool requires Event Manager role."""
        with client.session_transaction() as sess:
            sess['_user_id'] = str(temp_user.id)
        
        response = client.post(f'/pools/admin/add_member_to_pool/{temp_event.id}', data={
            'member_id': temp_user.id
        })
        assert response.status_code in [403, 302]
    
    def test_admin_add_member_to_pool_success(self, client, event_manager_user, temp_event_with_pool, temp_user):
        """Test admin can add member to pool."""
        with client.session_transaction() as sess:
            sess['_user_id'] = str(event_manager_user.id)
        
        with patch('app.audit.audit_log_create') as mock_audit:
            response = client.post(f'/pools/admin/add_member_to_pool/{temp_event_with_pool.id}', data={
                'member_id': temp_user.id
            })
    ...
    def test_admin_delete_from_pool_success(self, client, event_manager_user, temp_event_with_pool, temp_user):
    ...
    def test_admin_auto_select_pool_members_requires_role(self, client, temp_user, temp_event):
        """Test auto-select pool members requires Event Manager role."""
        with client.session_transaction() as sess:
            sess['_user_id'] = str(temp_user.id)
        
        response = client.post(f'/pools/admin/auto_select_pool_members/{temp_event.id}', data={
            'method': 'oldest_first',
            'count': 4
        })
        assert response.status_code in [403, 302]
    
    def test_admin_auto_select_pool_members_success(self, client, event_manager_user, temp_event_with_pool):
        ...
        with patch('app.audit.audit_log_bulk_operation') as mock_audit:
            response = client.post(f'/pools/admin/auto_select_pool_members/{temp_event_with_pool.id}', data={
                'method': 'oldest_first',
                'count': 4
            })
...
class TestPoolBusinessLogic:
    ...
    def test_pool_capacity_limits(self, client, temp_user, temp_event_with_pool):
    ...
    def test_pool_registration_with_event_dates(self, client, temp_user, temp_event_with_pool):
        """Test pool registration considers event timing."""
        with client.session_transaction() as sess:
            sess['_user_id'] = str(temp_user.id)
        
        # Registration should work for future events
        response = client.post(f'/pools/register/{temp_event_with_pool.pool.id}')
        assert response.status_code in [200, 302]
    
    def test_pool_member_status_validation(self, client, temp_event_with_pool):
    ...
    def test_pool_registration_audit_trail(self, client, temp_user, temp_event_with_pool):
        ...
        with patch('app.audit.audit_log_create') as mock_create:
            with patch('app.audit.audit_log_delete') as mock_delete:
                ...
                registration = PoolRegistration.query.filter_by(
                    pool_id=temp_event_with_pool.pool.id,
                    member_id=temp_user.id
                ).first()
...
@pytest.fixture
def temp_pool(app, temp_event):
    """Create a test pool."""
    with app.app_context():
        pool = Pool(event_id=temp_event.id, is_open=True)
        db.session.add(pool)
        db.session.commit()
        return pool
...
@pytest.fixture
def temp_event_with_pool(app, temp_event):
    """Create event with associated pool."""
    with app.app_context():
        pool = Pool(event_id=temp_event.id, is_open=True)
        temp_event.has_pool = True
        db.session.add(pool)
        db.session.commit()
        temp_event.pool = pool
        return temp_event